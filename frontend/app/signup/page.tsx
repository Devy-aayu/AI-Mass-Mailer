  ...
  return (
    <main className="auth_page">
      <section className="auth_aside">
        ...
        <span className="auth_stamp">Import · personalize · send · replay</span>
      </section>
      <section className="auth_form_side">
        <div className="auth_panel">
          ...
          <form onSubmit={submit}>
            <div className="field"><label className="field_label">Name</label><input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" autoComplete="name" /></div>
            ...
            <div className="field"><label className="field_label">Password</label><input className="input" type="password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} autoComplete="new-password" required placeholder="8+ chars, letter and number" /></div>
            {error && <div className="auth_error">{error}</div>}
            <button className="btn btn_primary" style={{ width: "100%" }} disabled={loading}>{loading ? "Creating workspace…" : "Create account"}</button>
            <p className="auth_footer">Already have an account? <Link href="/login">Sign in</Link></p>
          </form>
        </div>
      </section>
    </main>
